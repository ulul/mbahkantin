...
$fileExt = end($kaboom);
if (!is_int($harga) AND $harga <0) {
	$pesan = "ERROR: Harga harus Angka dan lebih dari 0";
	$err = 1;
}
if ($fileErrorMsg <1 ) {
if($fileSize > 5242880) { 
    ...
    $err = 1;
     ...
     $pesan ="ERROR: File bukan gambar berformat jpg / jpeg / png";
     unlink($fileTmpLoc); 
     $err = 1;
...
$old = Query::builder("SELECT * FROM menu WHERE ID_MENU=$id_menu");
unlink("upload/".$old[0]["GAMBAR"]);
$gambar = $new_name;
}
if ($err !=1) {
	...
	if ($fileErrorMsg > 0 ) {
	$gambar = null;
	$ad = Query::builder("UPDATE menu SET NAMA_MENU='$nama_menu', JENIS='$jenis', HARGA='$harga', ID_KANTIN='$kantin' WHERE ID_MENU='$id_menu'");

}else {
$ad = Query::builder("UPDATE menu SET NAMA_MENU='$nama_menu', JENIS='$jenis', HARGA='$harga', ID_KANTIN='$kantin', GAMBAR='$gambar' WHERE ID_MENU='$id_menu'");
}
}else{
	$pesan = $pesan;
	$err = 1;
...
$_SESSION["buka"] = Input::post("kantin", true);	
$_SESSION["pesan"] = "<div class='alert alert-success alert-dismissible fade in' role='alert'><button class='close' aria-label='Close' data-dismiss='alert' type='button'><span aria-hidden='true'>×</span></button><strong>Update Menu Berhasil</strong></div>";
header("location:kantin.php?page=manage_kantin");
}
?>