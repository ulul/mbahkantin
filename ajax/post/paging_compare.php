  ...
  if (count($_SESSION["compare"]) >0) {
      ...
      if (Input::post('limit')) {
        ...
        $limit = 1;
            ...
            $where =  implode(" || ID_MENU = ", $_SESSION["compare"]);
            $query = "SELECT * FROM menu_utama WHERE max_pesan !=0 AND STATUS_KANTIN =1 AND STATUS_MENU =1 AND ID_MENU = $where"; 
      ...
      <strong>Tidak ada menu yang anda bandingkan !</strong>
      </div>';
        }
           }else{
            echo '<div class="alert alert-danger alert-dismissible fade in" role="alert">
